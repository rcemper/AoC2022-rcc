const fs = require('fs');
const dir = "c:\\GitHub\\set1\\16\\" ;
const input = fs.readFileSync(dir+'data.txt').toString();
...
const part2 = () => {
 ...
 //                   console.log('we have a new p2 max', paths[i].releasedPressure+paths[j].releasedPressure );
                    console.log('part 2 = '+(paths[i].releasedPressure+paths[j].releasedPressure).toString());
                    max = paths[i].releasedPressure+paths[j].releasedPressure;
				} else { return; }
}
// p1;
console.log('part 1 = '+(computePaths(30)[0].releasedPressure).toString());
part2();
